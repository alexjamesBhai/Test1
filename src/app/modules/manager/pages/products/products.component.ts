  ...
  template: `
    <div class="space-y-6">
      ...
      <div *ngIf="showAddProductForm" class="bg-white rounded-lg shadow p-6">
        ...
        <form [formGroup]="productForm" (ngSubmit)="addProduct()" class="space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label class="block text-sm font-semibold text-slate-700 mb-2">Cost Price</label>
              <input
                type="number"
      ...
      <div class="bg-white rounded-lg shadow overflow-hidden">
        <table class="w-full">
          ...
          <tbody class="divide-y divide-slate-200">
            <tr *ngFor="let product of products" class="hover:bg-slate-50 transition">
              ...
              <td class="px-6 py-4 text-sm text-slate-900 font-semibold">{{ product.price | currency }}</td>
              <td class="px-6 py-4 text-sm text-slate-900">
                <span [class]="product.stock > 10 ? 'text-green-600' : 'text-red-600'" class="font-semibold">
                  {{ product.stock }} units
                </span>
              </td>
              <td class="px-6 py-4 text-sm">
                <span [class]="product.isActive ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-700'"
        ...
        <div *ngIf="products.length === 0" class="text-center py-8 text-slate-600">
          No products found
        </div>
      </div>
    </div>
...
export class ProductsComponent implements OnInit {
  ...
  addProduct(): void {
    ...
    this.apiService.createProduct(this.currentUser.organizationId, this.productForm.value).subscribe({
      next: (newProduct) => {
