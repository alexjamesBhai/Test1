    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-lg shadow p-6 border-l-4 border-blue-500">
          ...
          <p class="text-3xl font-bold text-slate-900 mt-2">{{ stats.totalSales | currency }}</p>
        </div>

        <div class="bg-white rounded-lg shadow p-6 border-l-4 border-green-500">
          <p class="text-slate-600 text-sm font-semibold">Active Products</p>
          <p class="text-3xl font-bold text-slate-900 mt-2">{{ stats.activeProducts }}</p>
        </div>

        <div class="bg-white rounded-lg shadow p-6 border-l-4 border-purple-500">
          <p class="text-slate-600 text-sm font-semibold">Team Members</p>
          <p class="text-3xl font-bold text-slate-900 mt-2">{{ stats.teamMembers }}</p>
        </div>

        <div class="bg-white rounded-lg shadow p-6 border-l-4 border-yellow-500">
          <p class="text-slate-600 text-sm font-semibold">Daily Revenue</p>
          <p class="text-3xl font-bold text-slate-900 mt-2">{{ stats.dailyRevenue | currency }}</p>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg shadow p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <button class="px-4 py-3 bg-green-500 hover:bg-green-600 text-white rounded-lg font-semibold transition">
            📦 View Products
          </button>
      ...
      <div class="bg-white rounded-lg shadow p-6">
        ...
        <p class="text-slate-600">Sales data will appear here once integrated with backend</p>
      </div>
    </div>
  `
})
